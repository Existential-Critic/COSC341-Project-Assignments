...
maxIconCount = Math.round(maxTrials/icons.length);
//Create connector array
iconConnector = new Array(icons.length);
for(var i=0;i<icons.length;i++){
  iconConnector[i] = {
    'id':icons[i].id,'count':0
  }
}
iconConnector.sort(function(a,b){return b.count-a.count});
  ...
  for(var i=0; i < icons.length; i++){
    icons[i].onclick = function(){
      ...
      for(var j=0;j<icons.length;j++){
        if(iconConnector[j].id[1] == iconClicked){
          iconConnector[j].count++;
          break;
        }
      }
      iconConnector.sort(function(a,b){return b.count-a.count});
      iconConnector.forEach(argIndex=>parent.appendChild(document.getElementById(argIndex.id)));
